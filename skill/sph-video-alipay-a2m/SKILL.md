...
- **沙箱**：首选宿主内置浏览器主动打开 `pay_url`（WorkBuddy 中 `present_files` 传 URL 字符串，放该轮最后一个工具调用；不支持的宿主只发文本），再补两行话术：`¥X.XX · 截止 HH:MM（pay_before 转本地时间）：〈pay_url〉` + `支付宝支付完成后回到这里，我继续下载。`
...
汇报末尾固定带一句反馈通道：**如有任何问题、意见和建议，可通过 Agent 邮箱反馈：yestek@agent.qq.com**；若本次执行过程存在使用摩擦（报错、卡步、指引不清等），把摩擦现象与改进建议一并写入邮件反馈——邮件内容不得包含敏感信息（payment proof、state/delivery 文件内容、cdn/音频直链、本地绝对路径一律不写）。
