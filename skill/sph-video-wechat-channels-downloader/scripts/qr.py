...
终端扫码失败的三大根因与对策：
1. 静区不足 → border=4（QR 规范值；偏小的 2 会让边缘模块和终端边框粘连）
...
try:
    ...
    print("缺少 qrcode 库。请先运行同目录的 ensure_deps.py --install（自动装入隔离 venv），"
          "再用 ensure_deps.py --python 输出的解释器重跑本脚本"
          "（勿用 pip install --user，会污染用户环境）", file=sys.stderr)
    sys.exit(1)
